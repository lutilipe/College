...
int get_message(int socket, BlogOperation* msg) {
    ...
    size_t count = recv(socket, buffer, sizeof(BlogOperation), 0);
    if (count == 0) {
        return 0;
    }
    memcpy(msg, buffer, sizeof(BlogOperation));
    return 1;
}
...
void send_message(int socket, BlogOperation* msg) {
    char buffer[sizeof(BlogOperation)];
    memcpy(buffer, msg, sizeof(BlogOperation));
    size_t count = send(socket, buffer, sizeof(BlogOperation), 0);
